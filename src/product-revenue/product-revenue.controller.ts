...
export class ProductRevenueController {
  constructor(private readonly productRevenueService: ProductRevenueService) {}
  ...
  async getTotalRevenue(@Id() adminId: string, @Query() params: RevenueDto) {
    ...
    const result = await this.productRevenueService.getTotalRevenue(
      adminId,
      startDate,
      endDate,
    );
  ...
  async getAdoptionRate(@Id() adminId: string, @Query() params: RevenueDto) {
    ...
    const result = await this.productRevenueService.getAdoptionRate(
      adminId,
      startDate,
      endDate,
    );
  ...
  @Get('top-products')
  async getTopProducts(@Id() adminId: string, @Query() params: RevenueDto) {
    const { startDate, endDate } = this.productRevenueService.validateDate(
      params.start,
      params.end,
    );
  ...
  @Get('top-users')
  async getTopUsers(@Id() adminId: string, @Query() params: RevenueDto) {
    const { startDate, endDate } = this.productRevenueService.validateDate(
      params.start,
      params.end,
    );
