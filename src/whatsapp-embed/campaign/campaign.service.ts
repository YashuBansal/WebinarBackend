...
import { Campaign, CampaignDocument } from '../../schemas/whatsapp-embed/campaign.schema';
...
export class CampaignService {
  ...
  async create(createCampaignDto: CreateCampaignDto, adminId: string): Promise<Campaign> {
   ...
   */
  async createCampaignWorkflow(createCampaignWorkflowDto: CreateCampaignWorkflowDto, adminId: string): Promise<any> {
    const { name, projectId, selectedContacts, templateName, variableMappings, sendType, scheduledAt } = createCampaignWorkflowDto;
    ...
    const campaign = new this.campaignModel({
      ...
      scheduledAt: sendType === 'scheduled' && scheduledAt ? new Date(scheduledAt) : undefined,
    });
    ...
    if (sendType === 'now') {
      try {
        ...
        return {
          ...
          executionResult,
          sampleMessage: this.generateSampleMessage('BODY', variableMappings, selectedContacts[0]),
   ...
   */
  private generateSampleMessage(templateBody: string, variableMappings: any[], sampleContact: any): string {
  ...
  async findAll(adminId: string, projectId?: string, page: number = 1, limit: number = 10): Promise<PaginatedCampaignsResponseDto> {
    const filter: any = { 
      adminId: new Types.ObjectId(adminId),
      isDeleted: false 
    };
  ...
  async update(id: string, updateCampaignDto: UpdateCampaignDto, adminId: string): Promise<Campaign> {
  ...
  async updateAnalytics(campaignId: string, analyticsData: any): Promise<Campaign> {
    const campaign = await this.campaignModel
      .findByIdAndUpdate(
        campaignId,
  ...
  async getCampaignAnalytics(campaignId: string, adminId: string): Promise<any> {
   ...
   */
  async executeCampaign(executeCampaignDto: ExecuteCampaignDto, adminId: string): Promise<any> {
    const { campaignId, contacts, bodyVariables, language } = executeCampaignDto;

    this.logger.log(`Executing campaign ${campaignId} for ${contacts.length} contacts`);
    ...
    if (campaign.status !== 'draft') {
      throw new BadRequestException('Campaign can only be executed when in draft status');
    }
  ...
  private async sendTemplateMessageToContact(
    ...
    const metaPayload = {
      ...
      template: {
        ...
        components: bodyVariables && bodyVariables.length > 0 ? [
          {
            type: 'body',
    ...
    // Remove components if no variables
    ...
    try {
      ...
      throw new InternalServerErrorException(
        (axiosError.response?.data as any)?.error?.message || 'Could not send template message.'
      );
    }
  }
  ...
  private async updateMessageStatus(
    ...
    try {
      ...
      await this.wabaMessageService.updateStatus(wabaMessageId, status, failureReason);

      // Get the message to find its campaign
      const message = await this.wabaMessageService.findByWabaMessageId(wabaMessageId);
      
      if (message) {
        // Update campaign analytics based on status
        await this.updateCampaignAnalyticsFromMessageStatus(message.campaignId.toString(), status);
      }
    } catch (error) {
      this.logger.error(`Failed to update message status for ${wabaMessageId}`, error);
    }
  }
   ...
   */
  private async updateCampaignAnalyticsFromMessageStatus(campaignId: string, status: string): Promise<void> {
    try {
      ...
      this.logger.error(`Failed to update campaign analytics for ${campaignId}`, error);
    }
  }
   ...
   */
  async getCampaignExecutionResults(campaignId: string, adminId: string): Promise<any> {
    const campaign = await this.findOne(campaignId, adminId);
